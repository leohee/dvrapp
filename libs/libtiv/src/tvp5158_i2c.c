...
/*
  Get video color related parameters
*/
Int32 Device_tvp5158GetVideoColor ( Device_Tvp5158Obj * pObj,
                                 Device_VideoDecoderColorParams * pPrm)
{
    Int32 status = 0;
    Device_VideoDecoderCreateParams *pCreateArgs;
    ...
    if ( status < 0 )
        return -1;
	pPrm->videoBrightness = regValue[0];
	pPrm->videoContrast = regValue[1];
	pPrm->videoSaturation = regValue[2];
	pPrm->videoHue = regValue[3];
    return status;
}
Int32 Device_tvp5158SetAutoContrastMode ( Device_Tvp5158Obj * pObj,
  						 AUTO_MODE* auto_contrast )
{
	Int32 status = 0;
    Device_VideoDecoderCreateParams *pCreateArgs;
    UInt8 regAddr[8];
    ...
    devId = auto_contrast->chId / DEVICE_TVP5158_CH_PER_DEVICE_MAX;
    chId = auto_contrast->chId% DEVICE_TVP5158_CH_PER_DEVICE_MAX;
    chId = 1 << chId;
	printf("Set channel %d auto contrast mode=%d\n",chId,auto_contrast->mode);
    /* There can be at max 2 TVP5158 devices per port */
    if ( devId >= DEVICE_TVP5158_DEV_MAX )
        return -1;
     ...
     * set video processing parameters
     */
    regAddr[numRegs] = DEVICE_TVP5158_REG_AUTO_CONTRAST;

   /* if ( auto_contrast->mode== 1 )
    {
        regValue[numRegs] = 0;     //auto mode
        numRegs++;
    }
    else if ( auto_contrast->mode== 2  )
    {
	 regValue[numRegs] = 2;     //user mode
        numRegs++;
    }
    else
    {
        regValue[numRegs] = 3;//disable
        numRegs++;
    }
*/
	regValue[numRegs] = auto_contrast->mode & 0x03;
   numRegs++;
    /*
     * write to TVP5158
     */
	...
	printf("addr=0x%x,regaddr=0x%x 0x%x,regvalue=%d %d,numRegs=%d\n",pCreateArgs->deviceI2cAddr[0],regAddr[0],regAddr[1],regValue[0],regValue[1],numRegs);
    if ( status < 0 )
        return -1;
