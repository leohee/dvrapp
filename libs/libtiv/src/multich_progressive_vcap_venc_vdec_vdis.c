...
Void MultiCh_createProgressiveVcapVencVdecVdis()
{
   ...
   // dupId[LIVE_DECODE_DUP_LINK_IDX]     = SYSTEM_VPSS_LINK_ID_DUP_1;
    ...
    capturePrm.isPalMode = TRUE;//Vcap_isPalMode();
#endif
...
#endif
        mergePrm[LIVE_DECODE_MERGE_LINK_IDX].outQueParams.nextLink        = gVdisModuleContext.swMsId[0];//dupId[LIVE_DECODE_DUP_LINK_IDX];
...
*/
    ...
    //{
        VDIS_DEV vdDevId = VDIS_DEV_HDMI;
         i=0;       
        swMsPrm[i].inQueParams.prevLinkId    = mergeId[LIVE_DECODE_MERGE_LINK_IDX];//dupId[LIVE_DECODE_DUP_LINK_IDX];
