...
static
Void  SystemLink_copySlaveCoreExceptionContext(UInt16 procId,
...
static
Void  SystemLink_handleSlaveCoreException(SystemLink_Obj *pObj, UInt32 procId)
{
    char *procName;

    procName = System_getProcName(procId);
    OSA_assert(procName != NULL);

    printf("\n\n%d:!!!SLAVE CORE [%s] DOWN!!!\n",OSA_getCurTimeInMsec(), procName);

    SystemLink_copySlaveCoreExceptionContext(procId,&gSlaveCoreExcInfo);
...
Int32 System_linkRegisterEventHandler(VSYS_EVENT_HANDLER_CALLBACK callback, Ptr appData)
{
