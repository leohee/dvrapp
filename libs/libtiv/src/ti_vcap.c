...
Int32 Vcap_autoContrast(char chId,char mode)
{
	int status;
...
Int32 Device_tvp5158ChromaBandwidth( Device_Tvp5158Obj * pObj,char chId,char level);
Int32 Vcap_start()
{
    UInt32 linkId, i;
    Int32  status = 0;
	...
	for (i = 0; i < VCAP_CHN_MAX; i++)
       	Vcap_autoContrast(i,0);//set auto contrast to 16 channel
    Vcap_setColor(110,100,180,0,0); //set channel 0 saturation is b0 to test
    Vcap_setColor(110,100,200,0,1);
    Vcap_setColor(110,100,220,0,2);
	//Vcap_setColor(110,100,220,0,3);
    Device_tvp5158ChromaBandwidth( gVcapModuleContext.tvp5158Handle[0],1,0x0d);
	//Device_tvp5158ChromaBandwidth( gVcapModuleContext.tvp5158Handle[0],2,0x0e);
	//Device_tvp5158ChromaBandwidth( gVcapModuleContext.tvp5158Handle[0],3,0x0f);
    return 0;
}
...
Int32 Vcap_getDynamicParamChn(VCAP_CHN vcChnId, VCAP_CHN_DYNAMIC_PARAM_S *psCapChnDynaParam, VCAP_PARAMS_E paramId)
{
    ...
    switch(paramId)
    {
        ...
        case VCAP_SCDGETALLCHFRAMESTATUS:
            ...
            break;
	 case VCAP_BRIGHTNESS:
	 	Vcap_getColor(NULL,&(psCapChnDynaParam->brightness),NULL,NULL,vcChnId);
		break;
	 case VCAP_SATURATION:
	 	Vcap_getColor(NULL,NULL,&(psCapChnDynaParam->satauration),NULL,vcChnId);
		break;
        default:
            status = ERROR_FAIL;
            break;
...
Int32 Vcap_setColor(Int32 contrast, Int32 brightness, Int32 saturation, Int32 hue, Int32 chId)
{
    ...
    if(gVcapModuleContext.vcapConfig.enableConfigExtVideoDecoder)
    {
        ...
        {
            printf
                ("CAPTURE: channel %d: Color parameter setting successful !!!videoBrightness=%d,videoContrast=%d,videoSaturation=%d,videoHue=%d\n",
                 chId,gVcapModuleContext.colorPrm.videoBrightness,gVcapModuleContext.colorPrm.videoContrast,
                 gVcapModuleContext.colorPrm.videoSaturation,gVcapModuleContext.colorPrm.videoHue);
        }
        else
        {
            printf
                ("CAPTURE: channel %d: Color parameter setting failed !!!videoBrightness=%d,videoContrast=%d,videoSaturation=%d,videoHue=%d\n",
                 chId,gVcapModuleContext.colorPrm.videoBrightness,gVcapModuleContext.colorPrm.videoContrast,
                 gVcapModuleContext.colorPrm.videoSaturation,gVcapModuleContext.colorPrm.videoHue);
        }
#endif
...
Int32 Vcap_getColor(Int32 *contrast, Int32 *brightness, Int32 *saturation, Int32 *hue, Int32 chId)
{
    ...
    if(gVcapModuleContext.vcapConfig.enableConfigExtVideoDecoder)
    {
        ...
        colorPrm.channelNum      = chId;
        colorPrm.videoBrightness = 0;//gVcapModuleContext.colorPrm.videoBrightness;
        colorPrm.videoContrast   = 0;//gVcapModuleContext.colorPrm.videoContrast;
        colorPrm.videoSaturation = 0;//gVcapModuleContext.colorPrm.videoSaturation;
        colorPrm.videoSharpness  = DEVICE_VIDEO_DECODER_NO_CHANGE;
        colorPrm.videoHue        = 0;//gVcapModuleContext.colorPrm.videoHue;

        status = Device_tvp5158Control(
                 gVcapModuleContext.tvp5158Handle[chId/VCAP_MAX_CHN_PER_DEVICE],
                 IOCTL_DEVICE_VIDEO_DECODER_GET_VIDEO_COLOR,
                 &colorPrm,
                 NULL);
        ...
        {
        	if(contrast != NULL)
			*(Int32*)contrast = colorPrm.videoContrast;
		if(brightness != NULL)
			*(Int32*)brightness = colorPrm.videoBrightness;
		if(saturation != NULL)
			*(Int32*)saturation = colorPrm.videoSaturation;
		if(hue != NULL)
			*(Int32*)hue = colorPrm.videoHue;
            printf
                ("CAPTURE: channel %d: Color parameter getting successful, brightness=%d !!!\n",
                 chId,colorPrm.videoBrightness);
        }
        else
        {
            printf
                ("CAPTURE: channel %d: Color parameter getting failed!!!\n",
                 chId);
        }
#endif
    }
...
Bool Vcap_isPalMode()
{
    if(gVcapModuleContext.vcapConfig.enableConfigExtVideoDecoder)
    {
        if(gVcapModuleContext.tvp5158Handle[0])
        {
            ...
            //Can set NTSC resolution later, whereas will enclink error
            return TRUE;
        }
        else
        {
