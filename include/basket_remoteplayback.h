/**
 @file basket_remoteplayback.h
 @defgroup BASKET_RMP Remote playback
 @ingroup BASKET
 @date 2010.4.8
 @brief
*/
...
/**
  @struct T_BKTRMP_CTRL
  @ingroup BASKET_RMP
  @brief value of remote playback control
*/
typedef struct
{
...
typedef struct {
	...
	int year;
	int mon;
	char *pRecDayTBL; ///< record day flag table as unit day.  31 days
}T_BKTRMP_RECDAYS_PARAM;

/**
  @struct T_BKTRMP_RECHOUR_PARAM
  @ingroup BASKET_RMP
*/
typedef struct {
	int ch;
	...
	int mon;
	int day;
	char *pRecHourTBL; ///< record hour flag table as unit minutes. 24*60
}T_BKTRMP_RECHOUR_PARAM;

/**
  @struct T_BKTRMP_PARAM
  @ingroup BASKET_RMP
*/
typedef struct
{
...
int  BKTRMP_GetRecHourDataAllCh(T_BKTRMP_CTRL* pRmpCtrl, int year, int mon, int day, char* pRecDayTBL); // 1440*BKT_MAX_CHANNEL minutes (24*60*BKT_MAX_CHANNEL)
