...
int  BKTRMP_Open(T_BKTRMP_CTRL* pRmpCtrl, long beginsec)
{
	...
	{
		TRACE1_BKTRMP("failed read index data. idd.id=0x%08x, idd.ch=%ld\n", idd.id, idd.ch);
		BKTRMP_Close(pRmpCtrl);
		return BKT_ERR;
	}
	bkt_pos = idd.fpos;
...
int  BKTRMP_GetRecHourDataUnion(T_BKTRMP_CTRL* pRmpCtrl, T_BKTRMP_RECHOUR_PARAM* prm)
{
	...
	int c,m;
	char cRecTblAll[TOTAL_MINUTES_DAY*BKT_MAX_CHANNEL];
	char path[LEN_PATH];
	...
	if(!READ_PTSZ(fd, cRecTblAll, TOTAL_MINUTES_DAY*BKT_MAX_CHANNEL))
	{
		TRACE1_BKTRMP("failed read rdb ch all data.\n");
		CLOSE_BKT(fd);
		return BKT_ERR;
	...
	else
	{
		...
		while(READ_ST(pRmpCtrl->hidx, idd))
		{
			if( (idd.ts.sec <= prev_sec && idd.ts.usec <= prev_usec)
				...
				|| idd.fpos <= bkt_fpos)
			{
				TRACE1_BKTRMP("Fount last record index pointer. idd.id=0x%08x, sec=%ld, usec=%ld, count=%d\n", idd.id, idd.ts.sec, idd.ts.usec, index_count);
				break;
			}
