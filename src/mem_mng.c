...
int MemMng_Init( MEM_MNG_INFO *pInfo )
{
	...
	for(i = 0; i < MEM_LAYOUT_NUM; i++)
	{
		mem_layout_setting[i].totalsizes = 0;
		if(i == MEM_LAYOUT_1)
		{
			for(cnt = 0; cnt < VIDOE_INFO_END; cnt++)
			{
				...
				{
					mem_layout_setting[i].profiles[cnt].cache_size 		= AUDIO_CACHE_SIZE;
					mem_layout_setting[i].profiles[cnt].cache_blk_size 	= AUDIO_CACHE_BLK_SIZE;
					mem_layout_setting[i].profiles[cnt].mem_size 		= AUDIO_MEM_SIZE;
					mem_layout_setting[i].profiles[cnt].mem_blk_size	= AUDIO_BLK_SIZE;
					mem_layout_setting[i].profiles[cnt].ext_size 		= 0;
				}
				else
				{
					mem_layout_setting[i].profiles[cnt].cache_size 		= MP4_CACHE_SIZE;
					mem_layout_setting[i].profiles[cnt].cache_blk_size 	= MP4_CACHE_BLK_SIZE;
					mem_layout_setting[i].profiles[cnt].mem_size 		= MP4_MEM_SIZE;
					mem_layout_setting[i].profiles[cnt].mem_blk_size 	= MP4_BLK_SIZE;
					mem_layout_setting[i].profiles[cnt].ext_size 		= MP4_1_EXTRA_SIZE;
				}
			}			
		}
//RTSP_EXT_CHANNEL		
		if(i == MEM_LAYOUT_2)
		{
			for(cnt = 0; cnt < VIDOE_INFO_END; cnt++)
			{
				...
				{
					mem_layout_setting[i].profiles[cnt].cache_size 		= MP4_CACHE_SIZE;//0x50000;
					mem_layout_setting[i].profiles[cnt].cache_blk_size 	= 5*1024;
					mem_layout_setting[i].profiles[cnt].mem_size 		= MP4_MEM_SIZE;//0x50000;
					mem_layout_setting[i].profiles[cnt].mem_blk_size 	= 5*1024;
					mem_layout_setting[i].profiles[cnt].ext_size 		= MP4_1_EXTRA_SIZE;
				}
			}
		}
	...
	for(cnt = 0; cnt < VIDOE_INFO_END; cnt++)
	{
		if(cnt >= AUDIO_INFO_G711_CH1)
		{
			Video_info_default[cnt].video_type 	= AUDIO_G711;
			Video_info_default[cnt].size		= AUDIO_MEM_SIZE;
			Video_info_default[cnt].extradata	= NULL;
			Video_info_default[cnt].extrasize	= 0;
			Video_info_default[cnt].IsCache		= 0;
			Video_info_default[cnt].blk_sz		= AUDIO_BLK_SIZE;
			Video_info_default[cnt].blk_num		= (AUDIO_MEM_SIZE/AUDIO_BLK_SIZE);
			Video_info_default[cnt].blk_free	= (AUDIO_MEM_SIZE/AUDIO_BLK_SIZE);
			Video_info_default[cnt].frame_num	= 0;
			Video_info_default[cnt].cur_frame	= -1;
			Video_info_default[cnt].cur_serial 	= -1;
			Video_info_default[cnt].cur_blk		= -1;
			Video_info_default[cnt].timestamp	= -1;
			Video_info_default[cnt].frame		= NULL;			
			Video_info_default[cnt].gop.last_Start 			= -1;
			Video_info_default[cnt].gop.last_Start_serial 	= -1;
			Video_info_default[cnt].gop.last_End 			= -1;
			Video_info_default[cnt].gop.last_End_serial 	= -1;
			Video_info_default[cnt].gop.lastest_I 			= -1;
			Video_info_default[cnt].gop.lastest_I_serial 	= -1;
		...
		{	
			Video_info_default[cnt].video_type 	= VIDEO_MP4;
			Video_info_default[cnt].size		= MP4_EXT_MEM_SIZE;
			Video_info_default[cnt].extradata	= NULL;
			Video_info_default[cnt].extrasize	= MP4_2_EXTRA_SIZE;
			Video_info_default[cnt].IsCache		= 0;
			Video_info_default[cnt].blk_sz		= MP4_EXT_BLK_SIZE;
			Video_info_default[cnt].blk_num		= (MP4_EXT_MEM_SIZE/MP4_EXT_BLK_SIZE);
			Video_info_default[cnt].blk_free	= (MP4_EXT_MEM_SIZE/MP4_EXT_BLK_SIZE);
			Video_info_default[cnt].frame_num	= 0;
			Video_info_default[cnt].cur_frame	= -1;
			Video_info_default[cnt].cur_serial 	= -1;
			Video_info_default[cnt].cur_blk		= -1;
	...
	printf(" ======== MEM_INIT layout Done... totalsize[%d] \n", pInfo->totalsize);
	
	shmem_id = shmget((key_t)pInfo->shmem_key, pInfo->totalsize, 0666|IPC_CREAT);
	if(shmem_id == -1)
	{
		__E(" MEM_INIT : shared memory fail shmem_id[%d] \n",shmem_id);
		goto MEM_INIT_FAIL;
	}
		
	pInfo->start_addr		= (int)shmat(shmem_id, 0, 0);//(unsigned long)OSA_cmemAlloc( pInfo->totalsize, 32);
